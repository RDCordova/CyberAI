...
#                     covariance_matrix = numeric_data.cov().values 
                    
#                     sample_data = multivariate_normal(mean=means, cov=covariance_matrix, size=num_rows)
#                     sample_df = pd.DataFrame(sample_data,columns=numeric_data.columns)
                    numeric_data = original_df.select_dtypes(include=[np.number])
        ...
        if st.session_state['generated_df'] is not None:
            ...
            if st.session_state['distribution_check'] is not None:
                st.write("### Distribution Quality Check")
                st.write("All test are evaluated against a p-value of 0.05. A p-value greater than 0.05 is a pass.\nOnly numeric and categorical values columns are checked.")
